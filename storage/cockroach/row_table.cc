...
void RowTable::init() {
  init_tree(&tree, 512 /*default_alloc_size*/, 0 /*memory_limit*/,
            sizeof(void*)/*size*/, RowTable::compare_rows, 1 /*with_delete*/,
...
bool RowTable::Get(cockroachdb::Slice &key, std::string *record, bool *found) {
  ROW_DATA **row_ptr;
...
int RowTable::compare_rows(const void* arg, const void *a, const void *b) {
  uchar *pa, *pb;
  size_t a_size, b_size;

  /* One of the parameters may be a cockroachdb slice */
  if (a == arg) {
    cockroachdb::Slice *slice= (cockroachdb::Slice*)a;
    pa= (uchar*)slice->data();
    a_size= slice->size();
  } else {
  ...
  /* Same as above for b */
  if (b == arg) {
    cockroachdb::Slice *slice= (cockroachdb::Slice*)b;
    pb= (uchar*)slice->data();
    b_size= slice->size();
  } else {
...
bool RowTable::Put(cockroachdb::Slice& key, cockroachdb::Slice& val) {
...
*/
bool RowTable::Delete(cockroachdb::Slice& key) {
  uchar *data = (uchar*)alloc_root(&mem_root, ROW_DATA_SIZE + key.size());
  ROW_DATA *rdata= (ROW_DATA*)data;
  rdata->key_len= key.size();
...
*/
void RowTable::rollback_stmt() {
  ROW_DATA *delete_list= NULL;
  RowTableIter iter(this);
...
RowTableIter::RowTableIter(RowTable *rtable_arg) :
  rtable(rtable_arg), row_ptr(NULL), change_id(rtable_arg->change_id)
{}


void RowTableIter::Seek(const cockroachdb::Slice &slice) {
...
bool RowTableIter::Valid() {
  return (row_ptr != NULL);
}


bool RowTableIter::is_tombstone() {
  DBUG_ASSERT(Valid());
  return ((*row_ptr)->value_len == DATA_IS_TOMBSTONE);
}


cockroachdb::Slice RowTableIter::key() {
  DBUG_ASSERT(Valid());
  return cockroachdb::Slice(((char*)*row_ptr) + ROW_DATA_SIZE, (*row_ptr)->key_len);
}


cockroachdb::Slice RowTableIter::value() {
  DBUG_ASSERT(Valid() && !is_tombstone());
  ROW_DATA *row= *row_ptr;
  return cockroachdb::Slice(((char*)row) + ROW_DATA_SIZE + row->key_len,
                        row->value_len);
}
