...
void CDBSE_KEYDEF::setup(TABLE *tbl)
{
  /*
...
/*
  ...
  @note
    ...
    cockroachdb SE supports "Extended keys". This means that PK columns are present
...
uint CDBSE_KEYDEF::get_primary_key_tuple(CDBSE_KEYDEF *pk_descr,
...
uint CDBSE_KEYDEF::pack_index_tuple(TABLE *tbl, uchar *packed_tuple,
...
void CDBSE_KEYDEF::successor(uchar *packed_tuple, uint len)
{
...
uint CDBSE_KEYDEF::pack_record(TABLE *tbl, const uchar *record,
                               uchar *packed_tuple,
...
int CDBSE_KEYDEF::unpack_record(TABLE *table, uchar *buf,
                                 const cockroachdb::Slice *packed_key,
                                 const cockroachdb::Slice *unpack_info)
{
  int res= 0;
...
void CDBSE_TABLE_DEF::write_to(CockroachDB *cdb, uchar *key, size_t keylen)
{
...
uchar* Table_ddl_manager::get_hash_key(CDBSE_TABLE_DEF *rec, size_t *length,
...
{
  CDBSE_TABLE_DEF* elem= (CDBSE_TABLE_DEF*)data;
  delete elem;
}


bool Table_ddl_manager::init(CockroachDB *cocdb)
{
  ...
  uchar ddl_entry[CDBSE_KEYDEF::INDEX_NUMBER_SIZE];
  ...
  for (it->Seek(ddl_entry_slice); it->Valid(); it->Next())
  {
    ...
    {
      sql_print_error("cockroachdb: Table_store: key has length %d (corruption?)",
                      (int)key.size());
      return true;
    }

    if (memcmp(key.data(), ddl_entry, CDBSE_KEYDEF::INDEX_NUMBER_SIZE))
      break;

    tdef->dbname_tablename.append(key.data() + CDBSE_KEYDEF::INDEX_NUMBER_SIZE,
                                  key.size() - CDBSE_KEYDEF::INDEX_NUMBER_SIZE);

    // Now, read the DDLs.

    if (val.size() < CDBSE_KEYDEF::INDEX_NUMBER_SIZE)
    {
      sql_print_error("cockroachdb: Table_store: no keys defined in %*s",
                      (int)key.size(), key.data());
      return true;
    }
    if (val.size() % CDBSE_KEYDEF::INDEX_NUMBER_SIZE)
    {
      sql_print_error("cockroachdb: Table_store: invalid keylist for table %s",
                      tdef->dbname_tablename.c_ptr_safe());
      return true;
    }
    tdef->n_keys= val.size() / CDBSE_KEYDEF::INDEX_NUMBER_SIZE;
    if (!(tdef->key_descr= (CDBSE_KEYDEF**)my_malloc(sizeof(CDBSE_KEYDEF*) *
                                                     tdef->n_keys,
                                                     MYF(MY_ZEROFILL))))
      return true;
    ...
    for (uint keyno=0; ptr < ptr_end; keyno++)
    {
      ...
      /*
        We can't fully initialize CDBSE_KEYDEF object here, because full
      ...
      */
      tdef->key_descr[keyno]= new CDBSE_KEYDEF(index_number, keyno);
...
CDBSE_TABLE_DEF* Table_ddl_manager::find(uchar *table_name,
                                         uint table_name_len,
                                         bool lock)
{
  CDBSE_TABLE_DEF *rec;
  ...
  rec= (CDBSE_TABLE_DEF*)my_hash_search(&ddl_hash, (uchar*)table_name,
                                        table_name_len);
  if (lock)
...
int Table_ddl_manager::put_and_write(CDBSE_TABLE_DEF *tbl, CockroachDB* cocdb)
{
  uchar buf[NAME_LEN * 2 + CDBSE_KEYDEF::INDEX_NUMBER_SIZE];
  uint pos= 0;

  store_index_number(buf, DDL_ENTRY_INDEX_NUMBER);
  pos+= CDBSE_KEYDEF::INDEX_NUMBER_SIZE;
...
int Table_ddl_manager::put(CDBSE_TABLE_DEF *tbl, bool lock)
{
  CDBSE_TABLE_DEF *rec;
  my_bool result;
  ...
  rec= (CDBSE_TABLE_DEF*)find((uchar*)tbl->dbname_tablename.c_ptr(),
...
void Table_ddl_manager::remove(CDBSE_TABLE_DEF *tbl, CockroachDB *cocdb, bool lock)
{
  if (lock)
    mysql_rwlock_wrlock(&rwlock);
...
{
  CDBSE_TABLE_DEF *rec;
  CDBSE_TABLE_DEF *new_rec;
  bool res= true;
  uchar new_buf[NAME_LEN * 2 + CDBSE_KEYDEF::INDEX_NUMBER_SIZE];
  uint new_pos= 0;

  mysql_rwlock_wrlock(&rwlock);
  if (!(rec= (CDBSE_TABLE_DEF*)find(from, from_len, false)))
    goto err;

  if (!(new_rec= new CDBSE_TABLE_DEF))
    goto err;
  ...
  new_pos+= CDBSE_KEYDEF::INDEX_NUMBER_SIZE;
