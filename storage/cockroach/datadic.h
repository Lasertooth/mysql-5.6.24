...
inline void store_index_number(uchar *dst, uint32 number)
{
    ...
    memcpy(dst, &number, CDBSE_KEYDEF::INDEX_NUMBER_SIZE);
#else
...
/*
  ...
  in-place. See ha_cockroachdb::convert_record_to_storage_format for details.
...
public:
  ...
  uint pack_record(TABLE *tbl, const uchar *record, uchar *packed_tuple,
                   ...
                   uint n_key_parts=0);
  int unpack_record(TABLE *table, uchar *buf, const cockroachdb::Slice *packed_key,
  ...
  */
  ...
  uint get_primary_key_tuple(CDBSE_KEYDEF *pk_descr, const char *key,
                             char *pk_buffer);
  ...
  CDBSE_KEYDEF(uint indexnr_arg, uint keyno_arg) :
...
private:
  ...
  friend class CDBSE_TABLE_DEF; // for index_number above
...
/*
  ...
  There is only one CDBSE_TABLE_DEF object for a given table.
  That's why we keep auto_increment value here, too.
*/

class CDBSE_TABLE_DEF
{
public:
  CDBSE_TABLE_DEF() : key_descr(NULL), auto_incr_val(1)
  {
  ...
  void write_to(CockroachDB *cdb, uchar *key, size_t keylen);
};
...
/*
  This contains a mapping of
     ...
     dbname.table_name -> array{CDBSE_KEYDEF}.
...
{
  HASH ddl_hash; // Contains CDBSE_TABLE_DEF elements
...
public:
  bool init(CockroachDB *cocdb);
  void cleanup();

  int put_and_write(CDBSE_TABLE_DEF *key_descr, CockroachDB *rdb_dict);
  int put(CDBSE_TABLE_DEF *key_descr, bool lock= true);
  void remove(CDBSE_TABLE_DEF *rec, CockroachDB *cocdb, bool lock=true);

  CDBSE_TABLE_DEF *find(uchar *table_name, uint len, bool lock=true);
...
private:
  static uchar* get_hash_key(CDBSE_TABLE_DEF *rec, size_t *length,
